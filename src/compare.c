...
int bbox_comparator(const void *a, const void *b)
{
    ...
    free_image(im2);
    free(X);
    if (predictions[iclass*2] > predictions[iclass*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int iclass, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[iclass] - a->elos[iclass])/400.));
    float EB = 1./(1+pow(10, (a->elos[iclass] - b->elos[iclass])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[iclass] += k*(SA - EA);
    b->elos[iclass] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int iclass)
{
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (iclass = 0; iclass < classes; ++iclass){
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", iclass);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[iclass]);
        }
        fclose(outfp);
    }
