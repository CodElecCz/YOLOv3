...
void delta_region_class(float *output, float *delta, int index, int iclass, int classes, tree *hier, float scale, int stride, float *avg_cat, int tag)
{
    int i, n;
    if(hier){
        float pred = 1;
    ...
    for (b = 0; b < l.batch; ++b) {
        if(l.softmax_tree){
            ...
            for(t = 0; t < 30; ++t){
                ...
                if(!truth.x) break;
                int iclass = net.truth[t*(l.coords + 1) + b*l.truths + l.coords];
                float maxp = 0;
                int maxi = 0;
                if(truth.x > 100000 && truth.y > 100000){
                    for(n = 0; n < l.n*l.w*l.h; ++n){
                        ...
                        float p = scale*get_hierarchy_probability(l.output + class_index, l.softmax_tree, iclass, l.w*l.h);
                        if(p > maxp){
                            maxp = p;
                            maxi = n;
                        }
                    ...
                    delta_region_class(l.output, l.delta, class_index, iclass, l.classes, l.softmax_tree, l.class_scale, l.w*l.h, &avg_cat, !l.softmax);
