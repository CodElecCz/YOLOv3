...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int iclass = max_index(test.y.vals[i], 10);
        if(index == iclass) avg_acc += 1;
        free_image(im);
        printf("%4d: %.2f%%\n", i, 100.*avg_acc/(i+1));
    }
}
...
const char *labels[] = {"airplane","automobile","bird","cat","deer","dog","frog","horse","ship","truck"};
    ...
    for(i = 0; i < train.X.rows; ++i){
        ...
        int iclass = max_index(train.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/train/%d_%s",i,labels[iclass]);
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        int iclass = max_index(test.y.vals[i], 10);
        char buff[256];
        sprintf(buff, "data/cifar/test/%d_%s",i,labels[iclass]);
        save_image_options(im, buff, PNG, 0);
    }
}
