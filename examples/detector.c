...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    char *train_images = option_find_str(options, "train", (char*)"data/train.list");
    char *backup_directory = option_find_str(options, "backup", (char*)"/backup/");
...
void print_imagenet_detections(FILE *fp, int id, detection *dets, int total, int classes, int w, int h)
{
    ...
    for(i = 0; i < total; ++i){
        ...
        for(j = 0; j < classes; ++j){
            int iclass = j;
            if (dets[i].prob[iclass]) fprintf(fp, "%d %d %f %f %f %f %f\n", id, j+1, dets[i].prob[iclass],
                    xmin, ymin, xmax, ymax);
        }
    }
}
...
void validate_detector_flip(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", (char*)"data/train.list");
    char *name_list = option_find_str(options, "names", (char*)"data/names.list");
    char *prefix = option_find_str(options, "results", (char*)"results");
    ...
    char buff[1024];
    char *type = option_find_str(options, "eval", (char*)"voc");
    FILE *fp = 0;
    FILE **fps = 0;
    int coco = 0;
    int imagenet = 0;
    if(0==strcmp(type, "coco")){
		if (!outfile) sprintf(outfile, "coco_results");// outfile = "coco_results";
        snprintf(buff, 1024, "%s/%s.json", prefix, outfile);
        fp = fopen(buff, "w");
        fprintf(fp, "[\n");
        coco = 1;
    } else if(0==strcmp(type, "imagenet")){
		if (!outfile) sprintf(outfile, "imagenet-detection");// outfile = "imagenet-detection";
        snprintf(buff, 1024, "%s/%s.txt", prefix, outfile);
        fp = fopen(buff, "w");
        imagenet = 1;
        classes = 200;
    } else {
		if (!outfile) sprintf(outfile, "comp4_det_test_");// outfile = "comp4_det_test_";
...
#ifdef OPENCV
            make_window((char*)"predictions", 512, 512, 0);
